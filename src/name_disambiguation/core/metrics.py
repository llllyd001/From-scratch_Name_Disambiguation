...
def b3_metrics(truth_groups, predicted_groups, all_ids):
    truth_label = {pid: i for i, group in enumerate(truth_groups) for pid in group}
    predicted_label = {}
    for i, group in enumerate(predicted_groups):
        for pid in group:
    ...
    precision = sum(
        intersections[(truth_label[pid], predicted_label[pid])]
        / predicted_counts[predicted_label[pid]]
        for pid in all_ids
    ) / len(all_ids)
    recall = sum(
        intersections[(truth_label[pid], predicted_label[pid])]
        / truth_counts[truth_label[pid]]
        for pid in all_ids
    ) / len(all_ids)
    f1 = 2 * precision * recall / (precision + recall) if precision + recall else 0
    return {"precision": precision, "recall": recall, "f1": f1}
